...
int ad5; //valor analógico do sensor de pressão (Válvula Dinâmica)(pistão/golpe)
  ...
  switch(conexao){
    ...
    case 'I':
      ...
      if(condConect == 1){
        ...
        //**********************************************************************************//
        //************************* VÁLVULA DINÂMICA 2 (camara de confinamento) ****************************//
        dinamica1DNIT134:
        while(true){
          if (Serial.available()>1){
        ...
        //**********************************************************************************//
        //************************** VÁLVULA DINÂMICA 2 (pistão/golpe) ***************************//
        dinamica2DNIT134:
        while(true){
          if (Serial.available()>1){
      ...
      if(condConect == 1){
        ...
        while(true){ //RESPONSAVEL EM COLETAR A FREQUENCIA DOS GOLPES//
          if(Serial.available()> 0){
            frequencia = Serial.parseInt();
            if(frequencia > 0){
              Serial.print("FREQ="); // 1hz ou 2hz...limitação do sistema
              Serial.println(frequencia);
              serialFlush();
              break;
            }
        ...
        while(true){ //RESPONSAVEL EM COLETAR O TIPO DA FUNÇÃO WAVE//
          if(Serial.available()> 0){
            ...
            if(tipoWave > 0){ //apenas funciona o 1 (cossenoidal)
              Serial.print("Wave=");
        ...
        while(true){ //RESPONSAVEL EM COLETAR A CARGA CÍCLICA// valor em mbar
          if(Serial.available()> 0){
            ...
            if(setpoint1 > 10){
              ...
              setpointA = setpoint1*4095/3300;   //valor em contagem para o máximo da carga ciclica
              serialFlush();
              break;
            }
            if(setpoint1 == -3){
        ...
        while(true){ //RESPONSAVEL EM COLETAR A PRESSAO DE CONTATO// variacao (266 ~ 1599) mbar
          if(Serial.available()>0){
            ...
            if(setpoint2 > 10){
              ...
              setpointB = setpoint2*4095/3300;   //valor em contagem setpoint pressão de contato
              digitalWrite(pinB, LOW);  //ativa o pinAplicador
              serialFlush();
              break;
            }
...
void imprimir2(){
  ad2 = adc.read(A8);
  ad5 = adc.read(A2); // pressão do golpe (pistão)
  //ad6 = analogRead(A10);
  ...
  Serial.print(",");
  Serial.print(ad5);            //pressão do pistão (contato + cossenoide)
  Serial.print(",");
  ...
  switch(frequencia){
    case 1:
      if(currentMillis - initialMillis < intervalo01){
        if(conditionEnsaio == 0){ //para a norma 134
          digitalWrite(pinA, LOW);  //ativa o pinA
        }
        if(conditionEnsaio == 1){ //dnit 135
          if(currentMillis - initialMillis > intervalo00){
            //digitalWrite(pinB, HIGH);  //desativa o pinB
          }
          i = int(currentMillis - initialMillis);
          //setpointC = (waveformsTable[tipoWave-1][i]*setpointA+setpointB)*4095/3300; //DAC0
          //analogWrite(DAC0, setpointC);
          setpointE = (waveformsTable[0][i]*setpointA+setpointB)*4095/3300; //DAC1 - pistão/ golpe
          analogWrite(DAC1, setpointE);
        }
        //Serial.println(currentMillis - initialMillis); //verificar tempo percorrido
      }
      if((currentMillis - initialMillis) > intervalo01  && (currentMillis - initialMillis) < intervalo09){
        ...
        if(conditionEnsaio == 1){ //norma 135
          if((currentMillis - initialMillis) > 900){
          //digitalWrite(pinB, LOW);  //ativa o pinB
          }
          setpointC = (setpointB)*4095/3300; //pressão de contato - pistão/ golpe
          //analogWrite(DAC0, setpointC);
